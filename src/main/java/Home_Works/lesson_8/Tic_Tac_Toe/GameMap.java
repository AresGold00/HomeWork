package Home_Works.lesson_8.Tic_Tac_Toe;
...
import java.util.Random;
    ...
    public class GameMap extends JPanel {
        ...
        private boolean MODE_VS_HUMAN() {
            gameField = GameMap.getInstance();
        ...
        private boolean checkWini(int dot, int winLength) {
            for (int y = 0; y < fieldSizeY; y++) {            // проверяем всё поле
                for (int x = 0; x < fieldSizeX; x++) {
                    if (checkLine(x, y, 1, 0, winLength, dot)) return true;    // проверка  по +х
                    if (checkLine(x, y, 1, 1, winLength, dot)) return true;    // проверка по диагонали +х +у
                    if (checkLine(x, y, 0, 1, winLength, dot)) return true;    // проверка линию по +у
                    if (checkLine(x, y, 1, -1, winLength, dot)) return true;    // проверка по диагонали +х -у
                }
            }
            return false;
